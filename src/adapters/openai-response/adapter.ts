...
export class OpenAIResponseAdapter extends BaseAdapter {
    ...
    /**
     * Helper method to safely extract tool debug information (using native Tool type)
     */
    private getToolDebugInfo(tools: Tool[] | undefined | null): any[] {
        if (!tools) return [];
        return tools.map((tool, index) => {
            // Safe extraction of properties using optional chaining
            return {
                index,
                type: tool.type || 'unknown',
                // For function tools, extract relevant properties
                ...(tool.type === 'function' ? {
                    function_name: tool.name || 'missing_name',
                    has_parameters: !!tool.parameters,
                    executionEnabled: true // Mark tools as execution-enabled in debug info
                } : {})
            };
        });
    }
