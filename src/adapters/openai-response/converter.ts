...
export class Converter {
    ...
    convertToOpenAIResponseParams(model: string, params: UniversalChatParams): Partial<ResponseCreateParams> { // Return partial native type
        ...
        if (params.responseFormat === 'json' || (params.jsonSchema && params.jsonSchema.schema)) {
            // Set up text format configuration
            openAIParams.text = {
                format: {
                    type: 'json_object'
                }
            ...
            if (params.jsonSchema && typeof params.jsonSchema.schema === 'string') {
                try {
                    const schema = JSON.parse(params.jsonSchema.schema);
                    (openAIParams.text?.format as any).schema = schema;
                } catch (error) {
