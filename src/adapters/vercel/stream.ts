...
import {
    ...
    type VercelChatCompletionChunk
} from './types.ts';
...
export class VercelStreamHandler {
    private readonly log = logger.createLogger({ prefix: 'VercelStreamHandler' });
    ...
    async *handleStream(stream: AsyncIterable<unknown>): AsyncGenerator<UniversalStreamResponse> {
        ...
        for await (const value of stream) {
            this.log.debug('Raw Vercel AI Gateway stream chunk:', value);
            const chunk = value as VercelChatCompletionChunk;
            if (chunk.usage) {
        ...
        yield {
            ...
            metadata: {
                ...
                providerState: accumulatedReasoning
                    ? createVercelReasoningState(accumulatedReasoning)
                    : undefined
            }
        };
    }
    ...
    private mapToolCallDeltas(
        deltas: VercelChatCompletionChunk['choices'][number]['delta']['tool_calls'],
