...
jest.unstable_mockModule('../../../../core/mcp/OAuthProvider.js', () => {
  return {
    __esModule: true,
...
beforeAll(async () => {
  const indexModule = await import('@modelcontextprotocol/sdk/client/index.js');
  ...
  const OAuthProviderModule = await import('../../../../core/mcp/OAuthProvider.js');
...
describe('MCPServiceAdapter', () => {
  ...
  describe('connectToServer', () => {
    ...
    it('should throw MCPConnectionError if transport fails to start', async () => {
      ...
      mockStdioClientTransport.mockImplementationOnce(() => mockTransport);
      ...
      mockClient_1.mockImplementationOnce(() => mockClient_1);
  ...
  describe('HTTP fallback strategy', () => {
    it('should fallback to SSE when StreamableHTTP fails with 404', async () => {
      ...
      mockStreamableHTTPClientTransport_1.mockImplementationOnce(() => ({
        start: jest.fn().mockResolvedValue(undefined),
        close: jest.fn().mockResolvedValue(undefined)
      }));

      // Mock Client to fail when connecting with StreamableHTTP
      mockClient_1.mockImplementationOnce(() => ({
  ...
  describe('disconnectServer', () => {
    it('should disconnect from a connected server', async () => {
      ...
      (adapter as any);
      (adapter as any).sdkClients.set('server', mockClient_1);
      ...
      expect(mockClient_1.close).toHaveBeenCalled();
  ...
  describe('OAuth support', () => {
    ...
    beforeEach(() => {
      ...
      mockStreamableHTTPClientTransport_1.mockImplementation(() => mockTransport);
      mockSSEClientTransport_1.mockImplementation(() => mockTransport);
      mockClient_1.mockImplementation(() => mockClient_1);

      // Set up spies
      connectSpy = jest.spyOn(mockTransport, 'start');
      clientConnectSpy = jest.spyOn(mockClient_1, 'connect');
  ...
  describe('convertToToolDefinition / createZodSchemaFromParameters', () => {
    ...
    const mockSdkClientWithTools = (tools: any[]) => {
      ...
      mockClient_1_1.mockImplementationOnce(() => mockClient_1_1);
      return mockClient_1_1; // Return the mock client for potential assertions
    };
