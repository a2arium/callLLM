...
import { shouldRetryDueToContent, FORBIDDEN_PHRASES } from "../../../../../core/retry/utils/ShouldRetryDueToContent.ts";
...
describe("shouldRetryDueToContent", () => {
  ...
  describe("with string inputs", () => {
    test("returns true if response is short and contains a forbidden phrase", () => {
      ...
      const result = shouldRetryDueToContent(response, 200);
      expect(result.shouldRetry).toBe(true);
      expect(result.reason).toContain('forbidden phrase');
    });
    ...
    test("returns true if response is long and contains a forbidden phrase", () => {
      ...
      const result = shouldRetryDueToContent(longResponse, 200);
      expect(result.shouldRetry).toBe(true);
      expect(result.reason).toContain('forbidden phrase');
    });
    ...
    test("returns false for long string response without forbidden phrases", () => {
      ...
      const result = shouldRetryDueToContent(longString, 200);
      expect(result.shouldRetry).toBe(false);
    });

    test("handles a string with only whitespace", () => {
      const whitespaceString = "    \t\n   ";
      const result = shouldRetryDueToContent(whitespaceString, 200);
      expect(result.shouldRetry).toBe(true);
      expect(result.reason).toBe('Response content is empty');
    });
  });
  ...
  describe("with different thresholds", () => {
    test("uses default threshold when not specified", () => {
      ...
      const result = shouldRetryDueToContent(response);
      expect(result.shouldRetry).toBe(true);
      expect(result.reason).toContain('too short');
    });
    ...
    test("applies custom threshold for string input", () => {
      ...
      const result = shouldRetryDueToContent(response, 10);
      expect(result.shouldRetry).toBe(false);
    });
