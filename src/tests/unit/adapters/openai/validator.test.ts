...
jest.unstable_mockModule('@/core/models/ModelManager.ts', () => ({
  ...
  ModelManager: jest.fn().mockImplementation(() => ({
    getModel: jest.fn() // Add other methods if needed by Validator
  }))
}));
...
//         validateUniversalTools: jest.fn() 
//     }))
// }));
...
describe('OpenAI Response Validator', () => {
  let validator: Validator; // Use the direct import
  let mockModelManagerInstance: jest.Mocked<InstanceType<typeof ModelManager>>;

  beforeEach(() => {
    // Now ModelManager is a mock constructor, so we call it to get an instance
    mockModelManagerInstance = new ModelManager('openai') as jest.Mocked<InstanceType<typeof ModelManager>>;
    validator = new Validator(); // Use direct constructor
    (validator as any).modelManager = mockModelManagerInstance; // Assign the mocked ModelManager instance
  });
  ...
  describe('validateParams', () => {
    ...
    describe('reasoning model validation', () => {
      ...
      it('should reject reasoning settings for non-reasoning models', () => {
        ...
        mockModelManagerInstance.getModel.mockReturnValue(nonReasoningModel);
