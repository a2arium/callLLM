...
async function main() {
    ...
    const simpleResponses = await caller.call(
        'Analyze this text with char chunking:',
        {
            data: charsLimitText,
            maxCharsPerChunk: 40,
    ...
    console.log(`Debug: Received ${simpleResponses.length} responses`);
    simpleResponses.forEach((response, index) => {
        const content = response.content;
        if (content != null) {
            console.log(`\n[Response ${index + 1}/${simpleResponses.length}]`);
            console.log(`Chunk length: ${content.length} chars`);
            console.log(content);
        }
    });
    console.log('\n');
